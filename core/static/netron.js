...
xmlhttp=new XMLHttpRequest();
	xmlhttp.open("GET",encodeURI("../deleteGraph?id=21"),true);
	xmlhttp.send();
var element;
var connections = [];
for (i = 0; i < this.elements.length; i++)
	{
	element = this.elements[i];
	//log(element.getContent());
	//log(element.getPageRectangle().x);
	//log(element.getPageRectangle().y);
	
	xmlhttp=new XMLHttpRequest();
	xmlhttp.open("GET",encodeURI("../process?id=21&caption="+element.getContent()+"&x="+element.getPageRectangle().x+"&y="+element.getPageRectangle().y),true);
	xmlhttp.send();
	...
	for (j = 0; j < element.connectors.length; j++)
		{
			...
			for (k = 0; k < connector.connections.length; k++)
			{
				...
				if(connection.from.owner.content != element.getContent){
					connections.push(connection);
					log(connection.from.owner.content);
					log(connection.to.owner.content);
					xmlhttp=new XMLHttpRequest();
					xmlhttp.open("GET",encodeURI("../processRel?caption="+element.getContent()+"&From="+connection.from.owner.content+"&To="+connection.to.owner.content),true);
					xmlhttp.send();
				}
				}
			}
